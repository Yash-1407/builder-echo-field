...
export const activitiesApi = {
  getActivities: async (params?: {
    ...
    return apiRequest<ActivitiesResponse>(`/activities${query ? `?${query}` : ''}`);
  },
  ...
  createActivity: async (data: CreateActivityRequest): Promise<ActivityResponse> => {
  ...
  updateActivity: async (id: string, data: UpdateActivityRequest): Promise<ActivityResponse> => {
    return apiRequest<ActivityResponse>(`/activities/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  ...
  bulkDeleteActivities: async (activityIds: string[]): Promise<SuccessResponse & { deletedCount: number }> => {
    return apiRequest<SuccessResponse & { deletedCount: number }>('/activities', {
      method: 'DELETE',
